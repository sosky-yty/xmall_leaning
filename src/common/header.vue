        ...
        <div class="w-box">
          ...
          <div class="right-box">
            <div class="nav-list">
              ...
              <router-link to="/"><a @click="changePage(4)">捐赠</a></router-link>
...
export default {
  ...
  methods: {
    ...
    delGoods (prodctId) {
      if (this.login) {
        cartDel({ UserId: getStore('userId'), prodctId }).then(res => {
    ...
    navFixed () {
      if (
        ...
        this.ADD_ANIMATION({
          carPositionL: this.positionL,
          carPositionT: this.positionT
        })
      } else {
    ...
    openProduct (prodctId) {
      window.open(
        '//' + window.location.host + '/#/goodsDetails?productId=' + prodctId
      )
    },
    _getNavList () {
      navList.then(res => {
