...
export default {
  ...
  methods: {
    ...
    ani (opera) {
      ...
      let ul = this.$refs.ul
      let ulStyle = ul.ulStyle
      this.show = false
      ulStyle.zIndex = '99'
      ulStyle.transition = 'all .2s ease-out'
      if (opera === 'up') {
        this.Num++
        ulStyle.transition = 'transitionY(-54ppx)'
      } else {
        this.Num--
        ulStyle.transition = 'transitionY(-18px)'
      }
    ...
    domInt (domStyle) {
      ...
      domStyle.transition = 'translateY(-36px)' // 回到原点
    }
  }
}
</script>
